...
@section('content')
    <h1>Thêm Đánh Giá</h1>
    <br>
    <form action="{{ route('nhanvien.danhgia.store') }}" method="POST">
        @csrf
        <div class="mb-3">
            ...
            <input type="text" class="form-control" id="id_nguoi_dung" name="id_nguoi_dung"
                value="{{ old('id_nguoi_dung') }}" placeholder="Nhập ID người dùng vào" required>
            @error('id_nguoi_dung')
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control" id="id_san_pham" name="id_san_pham"
                value="{{ old('id_san_pham') }}" placeholder="Nhập ID sản phẩm vào" required>
            @error('id_san_pham')
                <div class="alert alert-danger mt-2">{{ $message }}</div>
            @enderror
        </div>
        <div class="mb-3">
            ...
            <input type="number" class="form-control" id="danh_gia_sao" name="danh_gia_sao"
                value="{{ old('danh_gia_sao') }}" min="1" max="5" placeholder="Nhập số sao (1-5)" required>
            @error('danh_gia_sao')
                <div class="alert alert-danger mt-2">{{ $message }}</div>
            @enderror
        </div>
        <div class="mb-3">
            ...
            <textarea class="form-control" id="noi_dung" name="noi_dung" placeholder="Nhập nội dung đánh giá vào" required>{{ old('noi_dung') }}</textarea>
            @error('noi_dung')
                <div class="alert alert-danger mt-2">{{ $message }}</div>
            @enderror
        </div>
        <div class="mb-3">
            ...
            <input type="date" class="form-control" id="ngay_danh_gia" name="ngay_danh_gia"
                value="{{ old('ngay_danh_gia') }}" required>
            @error('ngay_danh_gia')
                <div class="alert alert-danger mt-2">{{ $message }}</div>
            @enderror
        </div>
